...
// The "Meridian Home" footer link used to point at the Cloudflare demo; the live
// home is now the locally-hosted /home launcher. Self-heal on read so editions
// authored before the switch update automatically — no manual edit needed.
const LEGACY_MERIDIAN_HOME = 'https://meridian-os.pages.dev/';
function rewriteFooterLinks(links) {
  return arr(links).map((l) => (l && l.href === LEGACY_MERIDIAN_HOME ? { ...l, href: '/home' } : l));
}
...
export function normalizeEdition(e) {
  ...
  return {
    ...
    footerLinks: rewriteFooterLinks(o.footerLinks),
  };
}
...
export function blankEdition(date, title) {
  ...
  return normalizeEdition({
    ...
    footerLinks: [{ label: 'Meridian Home', href: '/home' }],
  });
}
