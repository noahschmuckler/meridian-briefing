# meridian-briefing — register the auto-start scheduled task, running IN PLACE
# from this git clone (no file copying). This is the orange-device path: clone
# via GitHub Desktop, run this once; to update later you just Sync, then restart
# the task. Run it from inside the clone:
#
...
Write-Host ""
Write-Host "meridian-briefing task setup (in-place from clone)" -ForegroundColor Cyan
Write-Host "------------------------------------------------------------------" -ForegroundColor Cyan
Write-Host "Repo root: $Root"

# --- 1. Sanity: is this actually the meridian-briefing clone? ---
...
$verRaw = (& node --version).TrimStart('v')   # e.g. 20.20.2
...
if ($major -lt 20 -or ($major -eq 20 -and $minor -lt 6)) {
  Write-Host "ERROR: Node v$verRaw is too old. The task launches with --env-file (needs Node 20.6+)." -ForegroundColor Red
  exit 1
}
...
if (-not (Test-Path -LiteralPath $envPath)) {
  ...
  Write-Host "  ADMIN_PASSWORD_HASH / ADMIN_PASSWORD_SALT  (generate on the Linux box:" -ForegroundColor Yellow
  Write-Host "    npm run hash-password  -> paste the two printed lines)" -ForegroundColor Yellow
  Write-Host "  BRIEFING_DB  (default .\data\state.json is fine on orange)" -ForegroundColor Yellow
  Write-Host ""
} else {
...
Write-Host ""
if ($listening) {
  Write-Host "meridian-briefing is up at http://127.0.0.1:$Port" -ForegroundColor Green
  Write-Host "  Provider view: http://127.0.0.1:$Port/" -ForegroundColor Green
  Write-Host "  Editor:        http://127.0.0.1:$Port/admin" -ForegroundColor Green
} else {
  ...
  Write-Host "  Common cause: a BRIEFING_DB path whose parent folder doesn't exist yet." -ForegroundColor Yellow
}
Write-Host ""
